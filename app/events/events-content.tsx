                ...
                <div className="w-full md:w-3/5 p-8 flex flex-col">
                    <div className="mb-auto">
                        <span className="text-[#D4AF37] font-bold uppercase tracking-widest text-xs mb-2 block">
                             {format(new Date(selectedEvent.date), 'EEEE, MMMM do')} @ {formatTime(selectedEvent.time)}
                        </span>
                        <h3 className="font-heading text-4xl text-white mb-6 leading-none">
                            {selectedEvent.title}
                        </h3>
                        <div className="prose prose-invert prose-sm text-slate-300 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                    ...
                    <div className="mt-8 pt-6 border-t border-white/10 flex items-center justify-between">
                        <div>
                            <span className="block text-slate-500 text-xs uppercase font-bold">Availability</span>
                            <span className="text-xl font-bold text-white">
                                {selectedEvent.tickets > 0 ? `${selectedEvent.tickets} Tickets Left` : 'Sold Out'}
                            </span>
                        </div>
                        <button className="bg-[#D4AF37] hover:bg-white text-black font-bold py-3 px-8 rounded-lg transition-colors uppercase tracking-widest text-sm shadow-lg shadow-[#D4AF37]/20">
                            Get Tickets
                        </button>
                    </div>
                </div>
